...
if($stmt->rowCount() > 0){			// 帳號 rowCount() > 0, 等於1時代表已被註冊過, 因為rowCount()=0 是第一筆資料.
	header("Location: error_msg.php?error_code=1");
	exit();
}

if($password != $password_check){	// 密碼第二次打的不一樣的話跳出 error.
...
switch($birth_month){
	...
	case 2:
		// 非閏年&& 29天時  ==> date_error
		// ($birth_year%4==0 && $birth_year%100!=0) || $birth_year%400==0    其中一個成立 ==> True (閏年)
